...
bool DrawMathFunction(Camera2D camera, char *stringFunction, int curr,
                      int counta);
...
bool DrawMathFunction(Camera2D camera, char *stringFunction, int curr,
  ...
  double x_var = 0;
  Vector2 *points = (Vector2 *)malloc(width * sizeof(Vector2));
  if (!points)
    return false;
  ...
  if (userFunction) {
    ...
    // DrawText("code broke :(", (int)camera.target.x, (int)camera.target.y, 30,
    // RED);
    return false;
  }
  free(points);
...
void ScreenGrid(Camera2D camera, float spacing) {
  ...
  for (float x = startX; x <= bottomRight.x; x += spacing) {
    ...
    DrawLineEx((Vector2){x, topLeft.y}, (Vector2){x, bottomRight.y}, thickness,
               col);
  }
  ...
  for (float y = startY; y <= bottomRight.y; y += spacing) {
    ...
    DrawLineEx((Vector2){topLeft.x, y}, (Vector2){bottomRight.x, y}, thickness,
               col);
  }
}
#endif
